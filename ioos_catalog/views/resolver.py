...
def get_resolver_asset(asset_id):
    ...
    '''
    dataset = db.Dataset.find_one({u"uid":asset_id, u"services":{u"$elemMatch":{u"service_type":u"SOS"}}})
    if not dataset:
        return jsonify(error="Not Found"), 404
    return json.dumps(dataset, default=json_util.default), 200, {'Content-Type':'application/json'}
